...
public class CatRoam : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        // Subscribe to the OnHungerThresholdReached event
        GetComponent<CatEat>().OnHungerThresholdReached += HandleHungerThresholdReached;
    }
    ...
    private void Update()
    {
        ...
        if (hungry && foodBowl != null)
        {
            ...
            {
                if (foodBowl.GetComponent<FoodBowl>().foodAmount > 0)
                {
                    Debug.Log("Eating from food bowl...");
                    foodBowl.GetComponent<FoodBowl>().foodAmount--;
                }
            }
            else
            {
    ...
    private void HandleHungerThresholdReached(int threshold)
    {
        // Set the 'hungry' boolean to true when the hunger threshold is reached
